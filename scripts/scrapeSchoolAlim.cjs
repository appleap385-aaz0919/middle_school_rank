...
/**
 * 학교알리미 사이트 분석 및 데이터 추출
 * 학교알리미는 동적 사이트로, AJAX 요청을 통해 데이터를 가져옴
 */
...
/**
 * 지역별 매핑 테이블
 * regionData.js의 지역명을 학교알리미 파라미터로 변환
 */
...
/**
 * 학교알리미에서 특정 지역의 중학교 데이터 가져오기
 */
async function fetchSchoolData(sido, gungu) {
  try {
    // 학교알리미 AJAX 엔드포인트 (실제 API 파라미터는 분석 필요)
    const url = `${BASE_URL}/asil/api/school_list_ajax.jsp`; // 예시 엔드포인트
    ...
    const response = await axios.get(url, {
      ...
      headers: {
        ...
        'Referer': 'https://asil.kr/asil/sub/school_list.jsp',
      },
      timeout: 10000,
    });
    ...
    console.error(`데이터 가져오기 실패 (${sido} ${gungu}):`, error.message);
    return null;
  }
}

/**
 * HTML 테이블에서 학교 데이터 파싱
 */
function parseSchoolData(html) {
  ...
  $('table tr').each((index, element) => {
    ...
    if (cells.length >= 5) {
      ...
      const achievementText = $(cells[3]).text().trim(); // 보통학력이상 비율
      const admissionText = $(cells[4]).text().trim(); // 특목고 진학률

      // 순위 파싱
      const rank = parseInt(rankText) || 0;

      // 성취도 비율 파싱 (예: "97.6%" -> 97.6)
      const achievement = parseFloat(achievementText.replace('%', '')) || 0;

      // 진학률 파싱 (예: "1.8%" -> 1.8)
      const admission = parseFloat(admissionText.replace('%', '')) || 0;

      if (name && rank > 0) {
        schools.push({
          rank,
          name,
          type: '공립', // 기본값, 실제 데이터에서 추출 필요
          gender: '남녀공학',
          achievement,
          admission,
          hasRanking: true,
...
async function scrapeAllRegions() {
  ...
  for (const [regionKey, mapping] of Object.entries(REGION_MAPPING)) {
    process.stdout.write(`\r[${successCount + failCount}/${totalRegions}] ${regionKey} 처리 중...`);

    try {
      // 학교알리미 API 호출
      const data = await fetchSchoolData(mapping.sido, mapping.gungu);
      ...
      if (data) {
        ...
        if (schools.length > 0) {
          ...
          successCount++;
          console.log(`\r[${successCount + failCount}/${totalRegions}] ${regionKey}: ${schools.length}개교 완료`);
        } else {
          failCount++;
          console.log(`\r[${successCount + failCount}/${totalRegions}] ${regionKey}: 데이터 없음`);
        }
      } else {
        failCount++;
        console.log(`\r[${successCount + failCount}/${totalRegions}] ${regionKey}: 요청 실패`);
      }
      ...
      failCount++;
      console.error(`\r[${successCount + failCount}/${totalRegions}] ${regionKey}: 오류 - ${error.message}`);
    }
  }
  ...
  console.log(`총 학교 수: ${Object.values(results['2025']).reduce((sum, schools) => sum + schools.length, 0)}개교\n`);
...
async function testSingleRegion() {
  console.log('테스트: 강남구 데이터 스크래핑\n');

  const testRegion = '서울특별시 강남구';
  const mapping = REGION_MAPPING[testRegion];

  if (!mapping) {
    console.error('지역 매핑을 찾을 수 없습니다');
    return;
  }

  console.log(`${testRegion} (${mapping.sido} ${mapping.gungu}) 데이터 요청...\n`);

  try {
    const data = await fetchSchoolData(mapping.sido, mapping.gungu);

    if (data) {
      console.log('원본 데이터 (일부):');
      console.log(data.substring(0, 1000));

      const schools = parseSchoolData(data);
      console.log(`\n파싱된 학교 수: ${schools.length}`);
      console.log('\n상위 5개 학교:');
      schools.slice(0, 5).forEach(school => {
        console.log(`  ${school.rank}위: ${school.name} (성취도: ${school.achievement}%, 진학률: ${school.admission}%)`);
      });
    }
  } catch (error) {
